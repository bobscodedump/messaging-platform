    ...
    >
      <div className='mb-3 flex flex-wrap items-center gap-3 text-xs text-slate-600'>
        <a
          ...
          download
          className='rounded border border-slate-300 bg-white px-2 py-1 font-medium hover:bg-slate-50'
        >
          📥 Download Sample CSV
        </a>
        <span>•</span>
      ...
      <div className='mb-4'>
        ...
        {selectedTemplate && (
          <div className='mt-2 p-3 rounded border border-slate-200 bg-slate-50 text-sm'>
            <div className='font-medium mb-1 text-slate-900'>Template Content:</div>
            <div className='whitespace-pre-wrap text-slate-700'>{selectedTemplate.content}</div>
        ...
        <div className='mb-4 p-3 rounded bg-blue-50 border border-blue-200'>
          ...
          <div className='flex flex-wrap gap-1.5'>
            {variableColumns.map((varName) => (
              <span
                key={varName}
                className='group relative flex items-center rounded bg-slate-100 px-1.5 py-0.5 text-slate-700'
              >
                <code className='text-[11px] font-mono'>{`{{${varName}}}`}</code>
                <span className='pointer-events-none absolute left-0 top-full z-10 mt-1 w-max max-w-xs translate-y-0 whitespace-nowrap rounded border border-slate-300 bg-slate-900 px-2 py-1 text-[10px] font-medium text-white opacity-0 shadow-lg transition-opacity group-hover:opacity-100'>
        ...
        <div className='mb-4 p-3 rounded bg-yellow-50 border border-yellow-300'>
          ...
          <div className='flex flex-wrap gap-1.5 mb-2'>
            {missingVariables.map((varName) => (
              <code key={varName} className='px-2 py-0.5 rounded bg-yellow-100 text-[11px] font-mono text-yellow-900'>
                {`{{${varName}}}`}
              </code>
            ))}
          </div>
      ...
      {step === 'idle' && (
        <div
          ...
          className='flex flex-col items-center justify-center gap-3 rounded-md border border-dashed border-slate-300 bg-slate-50 p-6 text-center'
        >
          <div className='text-sm text-slate-600'>Drag and drop CSV or click to browse</div>
      ...
      {step === 'validating' && <div className='p-6 text-center text-sm text-slate-500'>Validating CSV…</div>}

      {step === 'preview' && (
        <div className='space-y-4'>
          {headerErrors.length > 0 && (
            <div className='rounded border border-red-300 bg-red-50 p-3'>
              <div className='font-medium text-red-800'>Header Errors</div>
          ...
          <div>
            <div className='mb-2 font-medium text-sm text-slate-700'>Preview ({rows.length} rows)</div>
              ...
              <table className='w-full text-sm'>
                <thead className='bg-slate-100 sticky top-0'>
                  <tr>
                    ...
                    <th className='px-3 py-2 text-left text-slate-600'>First Name</th>
                    <th className='px-3 py-2 text-left text-slate-600'>Last Name</th>
                    {variableColumns.map((col) => (
                      <th key={col} className='px-3 py-2 text-left text-slate-600'>
                        {col}
                      </th>
                    ))}
                ...
                <tbody>
                  {rows.slice(0, 200).map((r) => (
                    <tr key={r.index} className='border-t border-slate-200 hover:bg-slate-50'>
                      <td className='px-3 py-2 text-slate-500'>{r.index + 1}</td>
                      <td className='px-3 py-2 text-slate-900'>{r.raw.phoneNumber}</td>
                      <td className='px-3 py-2 text-slate-900'>{r.raw.firstName || '-'}</td>
                      <td className='px-3 py-2 text-slate-900'>{r.raw.lastName || '-'}</td>
                      {variableColumns.map((col) => (
            ...
            {rows.length > 200 && <div className='text-xs text-slate-500'>Showing first 200 rows…</div>}
          </div>
      ...
      {step === 'uploading' && <div className='p-6 text-center text-sm text-slate-500'>Sending messages…</div>}
        ...
        <div className='space-y-4'>
          {importMutation.data.data.errorCount === 0 ? (
            ...
            <div className='rounded border border-amber-300 bg-amber-50 p-4'>
              <div className='font-medium text-amber-800'>⚠ Partial Success</div>
          ...
          {importMutation.data.data.errors && importMutation.data.data.errors.length > 0 && (
            <div className='rounded border border-red-300 bg-red-50 p-3'>
              <div className='font-medium text-red-800 mb-2'>{importMutation.data.data.errors.length} Error(s)</div>
              <ul className='max-h-60 overflow-y-auto text-sm text-red-700 space-y-1'>
                {importMutation.data.data.errors.slice(0, 50).map((e: any, i: number) => (
                  <li key={i} className='font-mono text-xs'>
                    <span className='font-semibold'>Row {e.index + 1}</span>
                    {e.row && <span className='text-red-600'> ({e.row})</span>}: {e.error}
                  </li>
                ))}
                {importMutation.data.data.errors.length > 50 && (
                  <li className='text-slate-500'>+ {importMutation.data.data.errors.length - 50} more errors…</li>
                )}
              </ul>
            </div>
          )}
