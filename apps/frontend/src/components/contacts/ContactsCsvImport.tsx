...
function parseCsvClient(text: string) {
  ...
  for (let i=1;i<lines.length;i++) {
    ...
    if (raw.birthDate) {
      ...
      if (!norm) errors.push('birthDate invalid'); else raw.birthDate = norm; // normalized for preview
    }
...
function normalizeDate(val: string): string | undefined {
  ...
  m = t.match(/^(\d{1,2})[\/](\d{1,2})[\/](\d{2,4})$/); if (m){
    let [,mm,dd,yy] = m; let year = parseInt(yy,10); if (yy.length===2) year = year<=30?2000+year:1900+year; 
    const month = parseInt(mm,10), day=parseInt(dd,10);
    if (month>=1&&month<=12&&day>=1&&day<=31){ return `${year.toString().padStart(4,'0')}-${mm.padStart(2,'0')}-${dd.padStart(2,'0')}`; }
  }
  return undefined;
}
...
export function ContactsCsvImport({ companyId }: Props) {
  ...
  const reset = () => { setStep('idle'); setFile(null); setParseResult(null); setGlobalError(null); };

  const startValidation = async (f: File) => {
    setFile(f); setStep('validating'); setGlobalError(null);
    try {
  ...
  const onInputChange = (e: React.ChangeEvent<HTMLInputElement>) => {
    const f = e.target.files?.[0]; if (f) startValidation(f);
  };

  const handleDrop = useCallback((e: React.DragEvent<HTMLDivElement>) => { e.preventDefault(); const f = e.dataTransfer.files?.[0]; if (f) startValidation(f); }, []);
  const handleDragOver = (e: React.DragEvent<HTMLDivElement>) => e.preventDefault();

  const confirmUpload = () => {
    if (!file) return; setStep('uploading'); importMutation.mutate(file, { onSuccess:()=> setStep('done'), onError:()=> setStep('preview') });
  };
    ...
    <Card title='Bulk Import Contacts' description='Workflow: Upload → Validate → Preview → Confirm.'>
      {step==='idle' || step==='validating' ? (
        ...
        >
          ...
          <Button size='sm' onClick={()=>fileInputRef.current?.click()} disabled={step==='validating'} variant='secondary'>
      ...
      {step==='preview' && parseResult && (
        <div className='space-y-4'>
          ...
          <div className='text-xs text-neutral-600'>Rows: {rows.length} | Total field errors: {totalErrors}</div>
            ...
            <table className='min-w-full text-xs'>
              <thead className='bg-neutral-100 dark:bg-neutral-800 sticky top-0'>
                <tr>
                  ...
                  {REQUIRED_HEADERS.map(h=> <th key={h} className='px-2 py-1 text-left'>{h}</th>)}
              ...
              <tbody>
                {rows.slice(0,200).map(r=> (
                  <tr key={r.index} className={r.errors.length? 'bg-red-50 dark:bg-red-900/20': ''}>
                    ...
                    {REQUIRED_HEADERS.map(h=> <td key={h} className='px-2 py-1 whitespace-nowrap max-w-[140px] truncate'>{r.raw[h]||''}</td>)}
          ...
          <div className='flex items-center justify-between'>
            ...
            <div className='flex items-center gap-3'>
              ...
              <Button size='sm' onClick={confirmUpload} disabled={!file || importMutation.isPending}>Confirm Upload</Button>
            </div>
          </div>
        </div>
      )}
      ...
      {step==='uploading' && (
        <div className='flex flex-col items-center gap-2 py-6 text-sm'>
          ...
          <Button size='sm' variant='ghost' disabled>Working…</Button>
        </div>
      )}
      ...
      {step==='done' && importMutation.data && (
        <div className='space-y-3'>
          <div className='rounded-md border border-green-300 bg-green-50 p-3 text-sm text-green-700'>
            Imported {importMutation.data.createdCount} contacts. Backend reported {importMutation.data.errorCount} error(s).
          </div>
          {importMutation.data.errors.length>0 && (
            <div className='max-h-40 overflow-auto rounded-md border border-neutral-200 p-3 dark:border-neutral-800'>
              ...
              <ul className='space-y-1 text-xs'>
                {importMutation.data.errors.slice(0,50).map(er=> <li key={er.index} className='text-red-500'>Row {er.index+2}: {er.error}</li>)}
                {importMutation.data.errors.length>50 && <li className='text-neutral-500'>+ {importMutation.data.errors.length-50} more…</li>}
              </ul>
            </div>
          )}
          <div className='flex justify-end'>
            <Button size='sm' variant='secondary' onClick={reset}>Import Another</Button>
          </div>
        </div>
      )}
